...
const LeaderMember = ({member}: MemberSchema) => {
  return (
    <View
      className={`bg-white w-11/12 mx-auto mb-3 rounded-2xl px-2 ${
        member.id === "1" ? "mt-5" : ""
      } ${member.id === "4" ? "mb-5" : ""}`}
    >
      <View className="flex-row justify-between items-center p-1">
        <View className="flex-row justify-between items-center">
          <View className="rounded-full w-6 h-6 border border-gray-500 flex-row justify-center items-center">
            <Text className="text-black text-center items-center">
              {member.id}
            </Text>
          </View>
          <Image
          ...
          <View className="mb-2">
            ...
            <Text className="text-red-500 text-lg text-bold">
              {member.point} points
            </Text>
          </View>
        </View>
        {member.id == "1" ? (
          <GoldCupIcon width={40} height={40} />
        ) : member.id == "2" ? (
          <SilverCupIcon width={40} height={40} />
        ) : member.id == "3" ? (
          <BronzeCupIcon width={40} height={40} />
        ) : (
          <View></View>
        )}
      </View>
